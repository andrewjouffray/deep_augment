	...
	if model.startswith("auto"):
		...
		for image_path in IMAGE_PATHS:
			...
			for idx, gt_box in enumerate(ground_truth_boxes):
				...
				# if not predictions overlapped then the model did not find that obejct
				if not is_found:

					cv2.rectangle(image_np,(int(gt_box[0]),int(gt_box[1])),(int(gt_box[2]),int(gt_box[3])),(255,0,0),3)	
					cv2.putText(image_np, "M", (int(gt_box[0]), int(gt_box[1])), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
...
with open("/home/ajouffray/TF-Object-Detection/testing/"+name+"/fold"+fold+"/IoU_accuracy.txt", 'w') as f:
	for k, v in ordered.items(): f.write('%s:%s\n' % (k, v)) 
 
with open("/home/ajouffray/TF-Object-Detection/testing/"+name+"/fold"+fold+"/PF_accuracy.txt", 'w') as f:
