...
# get the model
PATH_TO_SAVED_MODEL = os.path.join(args.model,"saved_model")
checkpoint_name = args.model.split("/")[-2]
...
for vid in videos:
	...
	if vid.lower().endswith(".avi") or vid.lower().endswith(".mp4") or vid.lower().endswith(".mov"):
		...
		side_size = ((int(width)*2), int(height))
		...
		com_writer = cv2.VideoWriter(save_path+'/output'+str(file_count)+'_'+checkpoint_name+'_com.avi', cv2.VideoWriter_fourcc(*'MJPG'),fps, size)

		side_writer = cv2.VideoWriter(save_path+'/output'+str(file_count)+'_'+checkpoint_name+'_side_by_side.avi', cv2.VideoWriter_fourcc(*'MJPG'),fps, side_size)
		...
		while True:
			...
			for mask in masks:
				...
				if score >= min_score:
					...
					if ratio >= min_ratio:
						...
						side_by_side = cv2.hconcat([image_np, out])
			...
			if not called:
				side_by_side = cv2.hconcat([image_np, blk])
				side_writer.write(side_by_side)
		...
		cap.release()
		com_writer.release()
		print("frames in video:", count, "saved frames", saved_frames)
